...
double vectorfield_2D_NR(double kf, double couplings[10], double t, double eps, double fields[2]) {
    ...
    double partial_gww_gww, partial_gww_gpp, partial_gpp_gpp, partial_gpp_gww;
    ...
    while (abs(y)>eps) {
        ...
        in = in + 1;
        if (in > 500) {
            cout << "failed to converge on solution for 2Dvector NR" << endl;
            exit(0);
        }
...
double tools :: scalarfield_2D_NR(double kf, double couplings[10], double t, double eps, double fields[2]) {
    ...
    while (abs(y)>eps) {
        ...
        in = in + 1;
        if (in > 500) {
            cout << "failed to converge on solution for 2D NR" << endl;
            exit(0);
        }
        //cout << mstarp << "  " << mstarn << endl;
    }
...
double tools :: get_t_betaeq(double kf, double couplings[10], double t_max) {
    double y,min, max;
    double x = 0.001; double sgn = 1.0; bool flag_zero = false;
    int n_array = 1000;
